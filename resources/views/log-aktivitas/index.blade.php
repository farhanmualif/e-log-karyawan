...
<div class="p-6 pb-8">
    ...
    <div class="mb-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <!-- Title -->
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Log Aktivitas Harian</h1>
                <p class="text-sm text-gray-600 mt-1">Kelola log aktivitas harian karyawan</p>
            </div>

            <a href="{{ route('log-aktivitas.create') }}" class="flex items-center gap-2 px-3 py-1.5 bg-teal-600 text-white rounded-md hover:bg-teal-700 transition-colors text-sm font-medium whitespace-nowrap">
        ...
        <form method="GET" action="{{ route('log-aktivitas.index') }}" class="bg-white p-3 rounded-lg border border-gray-200 mb-6">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div>
                    ...
                    <input type="date" name="tanggal_dari" value="{{ request('tanggal_dari') }}" class="w-full px-1.5 py-1 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent">
                </div>

                <!-- Filter Tanggal Sampai -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Tanggal Sampai</label>
                    <input type="date" name="tanggal_sampai" value="{{ request('tanggal_sampai') }}" class="w-full px-1.5 py-1 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent">
                </div>

                <!-- Filter Status -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Status</label>
                    <select name="status" class="w-full px-1.5 py-1 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent bg-white">
                ...
                <div>
                    ...
                    <input type="text" name="nama_karyawan" value="{{ request('nama_karyawan') }}" placeholder="Cari nama..." class="w-full px-1.5 py-1 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent">
                </div>
                @else
                <div></div>
                @endif
            ...
            <div class="flex gap-2 mt-4">
                <button type="submit" class="px-2 py-1 bg-teal-600 text-white rounded-md hover:bg-teal-700 transition-colors text-xs font-medium">
                    <i data-lucide="filter" class="w-3 h-3 inline mr-1"></i>
                    Filter
                </button>
                <a href="{{ route('log-aktivitas.index') }}" class="px-2 py-1 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300 transition-colors text-xs font-medium">
                    Reset
                </a>
            </div>
        </form>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-900">
                            <div class="flex flex-col gap-1">
                                <div class="font-medium">{{ $log->total_aktivitas ?? 0 }} aktivitas</div>
                    ...
                    @empty
                    <tr>
                        <td colspan="{{ in_array(Auth::user()->role, ['spv', 'manager', 'sdm', 'superadmin']) ? (Auth::user()->role != 'karyawan' ? '7' : '6') : (Auth::user()->role != 'karyawan' ? '6' : '5') }}" class="px-4 py-2 text-center text-sm text-gray-500">
